...
# ---------------------------------------------------------------------------
# Fyers quantity semantics (must match the app engine's execution_engine):
#
#   * MCX commodity futures   -> /order and /multiorder/margin take qty in
#     CONTRACTS (lots). qty=1 = one contract, pass straight through.
#   * NFO equity/index FUT    -> qty is UNDERLYING SHARES. One contract of a
#     lot-size-750 future is qty=750. Sending qty=1 is rejected by Fyers with
#     "-50 ... not a multiple of minimum lot size".
#   * NSE cash equity         -> shares, pass straight through.
#
# All strategy sizing upsteam is done in LOTS; these helpers convert a lot
# quantity into the qty the Fyers API actually expects so placements and the
# pre-trade margin check agree.
# ---------------------------------------------------------------------------
def is_equity_future(symbol: str = "", segment: str = "") -> bool:
    sym = (symbol or SYMBOL).upper()
    seg = (segment or SEGMENT).upper()
    return (seg.startswith("EQUITY") and "FUT" in sym) or (
        sym.startswith("NSE:") and "FUT" in sym and "-EQ" not in sym
    )


def broker_qty(qty: int, symbol: str = "", lot_size: int = 0) -> int:
    """Convert a strategy LOTS quantity into the Fyers-order `qty` units."""
    if is_equity_future(symbol):
        return max(1, int(qty)) * max(1, int(lot_size or LOT_SIZE))
