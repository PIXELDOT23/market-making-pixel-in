...
class CostAwareMarketMaker:
    ...
    def place_limit_order(self, side: int, price: float) -> str | None:
        ...
        order_data = {
            "symbol": self.symbol,
            # self.qty is a LOT quantity; NFO equity futures need underlying
            # SHARES so the broker accepts the order (qty=few shares is
            # rejected: "-50 not a multiple of minimum lot size").
            "qty": config.broker_qty(self.qty, self.symbol, self.lot_size),
            "type": 1,  # Limit Order
