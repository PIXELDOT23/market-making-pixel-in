...
def place_limit(fyers, side: int, price: float, risk: RiskManager) -> Optional[str]:
    ...
    data = {
        "symbol": config.SYMBOL,
        # LOT quantity -> broker units: NFO equity futures need underlying
        # shares (lots * LOT_SIZE), commodities/cash pass through as lots
        "qty": config.broker_qty(config.QUOTE_QTY, config.SYMBOL),
        "type": 1,  # limit order
